...
class TestPerceptron(unittest.TestCase):
    ...
    def test(self):
        ...
        self.assertEqual(
            type(perceptron), Perceptron, "Simple Initialization has failed."
        )
        self.assertEqual(
            type(perceptron.weigths),
            type(np.array([])),
            "List was not converted to Numpy array.",
        )
        assert_equal(perceptron.weigths, empty_array, "Numpy Arrays are not equal.")

        with self.assertRaises(
            ArrayMismatchException, msg="Array Match is not being checked."
        ):
            perceptron.predict(wrong_array)

        self.assertEqual(
            perceptron.predict([]), 3, "Compute is not taking bias into consideration."
        )

        self.assertEqual(
            second_perceptron.predict(input_values),
            -6,
            "Compute matrix multiplication is failing.",
        )
